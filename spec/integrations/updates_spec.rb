...
RSpec.describe "updates" do
  ...
  it "sets current strata record's upper bound validity to the current time and creates a new strata record" do
    ...
    expect(strata_book_class.count).to eq(2)
    expect(strata_book_class.find_by(title: "The Great Gatsby")).to have_attributes(
      pages: 180,
      validity: insert_time...update_time
    )
    expect(strata_book_class.find_by(title: "The Greatest Gatsby")).to have_attributes(
      pages: 180,
  ...
  context "when two updates are made in a single transaction" do
    it "creates two strata records with the first having an empty validity range" do
      ...
      expect(strata_book_class.count).to eq(3)
      expect(strata_book_class.find_by(title: "The Great Gatsby")).to have_attributes(
        title: "The Great Gatsby",
        pages: 180,
        validity: insert_time...update_time
      ...
      expect(strata_book_class.find_by(title: "The Absolutely Greatest Gatsby")).to have_attributes(
